...
import { Menu, Eye, Trash } from "lucide-react";
...
export default function EquipmentCard({
  ...
  const handleAvailabilityToggle = async () => {
    const state = itm.isAvailable ? "unavailable" : "available";
    let resp = await apiClient.put(`admins/equipments/${itm.id}/${state}`);
    return resp.data;
  };
  ...
  return (
    ...
    >
      <figure className="relative h-56 w-full">
        <img
          src={
            itm.imageUrls[0]?.url || "https://picsum.photos/400/200?random=1"
          }
          alt={itm.name}
          ...
          <div className="form-control">
            <label className="label cursor-pointer gap-2 bg-base-100/80 backdrop-blur-sm rounded-full px-4 py-2 shadow-lg">
              <span className="label-text text-sm font-bold">
                {itm.isAvailable ? "Available" : "Unavailable"}
              </span>
              <input
                type="checkbox"
                className="toggle toggle-sm toggle-success"
                checked={itm.isAvailable}
                onChange={() => {
                  toast.promise(mutate.mutateAsync(handleAvailabilityToggle), {
                    loading: "Updating availability...",
                    error: extract_message,
                    success: (data) => data.message || "Availability updated!",
                  });
                }}
              />
            </label>
      ...
      <div className="card-body p-6">
        ...
        <div className="badge badge-sm badge-primary">{itm.category.name}</div>
